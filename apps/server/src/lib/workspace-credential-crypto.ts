...
export async function encryptWorkspaceCredentialValue(
  ...
  const key = await deriveWorkspaceCredentialKey();
...
export async function decryptWorkspaceCredentialValue(
  ...
  if (payload.algorithm !== ENCRYPTION_ALGORITHM) {
    throw new Error(`Unsupported workspace credential algorithm: ${payload.algorithm}`);
  }
...
async function deriveWorkspaceCredentialKey(): Promise<Buffer> {
  return getWorkspaceCryptoKey();
}
